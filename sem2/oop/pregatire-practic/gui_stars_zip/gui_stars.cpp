...
void AstronautWindow::setup()
{
    ...
    auto* diameterLabel = new QLabel("DIAM: ");
    ...
    newStarLayout->addWidget(diameterLabel);
    newStarLayout->addWidget(diameterEdit);
    newStarLayout->addWidget(addStarButton);
    
    viewButton = new QPushButton("View Selected Star");
    newStarLayout->addWidget(viewButton);
    ...
    QObject::connect(viewButton, &QPushButton::clicked, this, &AstronautWindow::viewStars);
    
 }

void AstronautWindow::viewStars() {
    auto selectedRows = table->selectionModel()->selectedRows();
    if (!selectedRows.empty()) {
        int selectedRow = selectedRows.first().row();
        Star selectedStar = this->stars->getStars().at(selectedRow);
        auto* viewStarsWindow = new ViewStarsWindow(this->service, this->aust.getConstellation(), selectedStar);
        viewStarsWindow->show();
    }
    else {
